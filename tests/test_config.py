...
def test_http_transport_gets_defaults():
    config = ServerConfig.from_env({"MCP_TRANSPORT": "http"})
    assert config == ServerConfig(transport="http", host="127.0.0.1", port=8000)
...
def test_http_host_and_port_resolve_through():
    ...
    assert config == ServerConfig(transport="http", host="192.168.1.50", port=9000)


def test_http_non_integer_port_raises():
    with pytest.raises(ValueError, match="MCP_PORT"):
        ServerConfig.from_env({"MCP_TRANSPORT": "http", "MCP_PORT": "abc"})


@pytest.mark.parametrize("port", ["70000", "0"])
def test_http_out_of_range_port_raises(port):
    with pytest.raises(ValueError, match="1-65535"):
        ServerConfig.from_env({"MCP_TRANSPORT": "http", "MCP_PORT": port})
